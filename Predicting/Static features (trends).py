...
data = pd.DataFrame(index=data_val['video_id'], columns=['Prediction'])

X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size=0.5, random_state=6)
X_test, X_valid, y_test, y_valid = train_test_split(X_test, y_test, train_size=0.5, random_state=6)
...
model = BaggingRegressor(random_state=21, n_estimators=searcher.best_params_["n_estimators"],
